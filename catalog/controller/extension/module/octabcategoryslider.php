...
class ControllerExtensionModuleOctabcategoryslider extends Controller {
	public function index($setting) {
		...
		foreach ($arrayCates as $cate_id) {
			...
			if(isset($categories_info['description'])) {
				$category_description = $categories_info['description'];
				$arrayCateName[$cate_id]['description'] = utf8_substr(strip_tags(html_entity_decode($category_description, ENT_QUOTES, 'UTF-8')), 0, 200) . '..';
				$arrayCateName[$cate_id]['description2'] = utf8_substr(strip_tags(html_entity_decode($category_description, ENT_QUOTES, 'UTF-8')), 0, 50) . '..';
			}
			if($use_thumbnail) {
				...
				if(isset($categories_info['homethumb_image'])) {
					$arrayCateName[$cate_id]['homethumb_image'] = $this->model_tool_image->resize($categories_info['homethumb_image'], 300, 429);
				} else {
		...
		$data['numoftab'] = ($data['module']%2==0) ? " even" : " odd";
